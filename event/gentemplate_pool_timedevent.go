// autogenerated: do not edit!
// generated from gentemplate [gentemplate -d Package=event -id timedEvent -d PoolType=timedEventPool -d Type=timedEvent -d Data=events github.com/platinasystems/elib/pool.tmpl]

package event

import (
	"github.com/platinasystems/elib"
)

type timedEventPool struct {
	elib.Pool
	events []timedEvent
}

func (p *timedEventPool) GetIndex() (i uint) {
	l := uint(len(p.events))
	i = p.Pool.GetIndex(l)
	if i >= l {
		p.Validate(i)
	}
	return i
}

func (p *timedEventPool) PutIndex(i uint) (ok bool) {
	return p.Pool.PutIndex(i)
}

func (p *timedEventPool) IsFree(i uint) (ok bool) {
	return p.Pool.IsFree(i)
}

func (p *timedEventPool) Resize(n uint) {
	c := elib.Index(cap(p.events))
	l := elib.Index(len(p.events) + int(n))
	if l > c {
		c = elib.NextResizeCap(l)
		q := make([]timedEvent, l, c)
		copy(q, p.events)
		p.events = q
	}
	p.events = p.events[:l]
}

func (p *timedEventPool) Validate(i uint) {
	c := elib.Index(cap(p.events))
	l := elib.Index(i) + 1
	if l > c {
		c = elib.NextResizeCap(l)
		q := make([]timedEvent, l, c)
		copy(q, p.events)
		p.events = q
	}
	if l > elib.Index(len(p.events)) {
		p.events = p.events[:l]
	}
}

func (p *timedEventPool) Elts() int {
	return len(p.events) - p.FreeLen()
}
